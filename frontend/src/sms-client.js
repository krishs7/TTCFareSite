...
export async function scheduleSmsReminders(deadlineISO) {
  ...
  // Normalize to a canonical ISO string with timezone (Z) to avoid server parse surprises.
  let normalized = deadlineISO;
  try {
    const d = new Date(deadlineISO);
    if (!Number.isNaN(d.getTime())) normalized = d.toISOString();
  } catch {}

  const res = await fetch(`${API_BASE}/api/sms/reminders`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ recipientId, deadlineISO: normalized })
  });
